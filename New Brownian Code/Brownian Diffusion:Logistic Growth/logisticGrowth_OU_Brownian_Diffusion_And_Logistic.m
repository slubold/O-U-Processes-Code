function uprime = logisticGrowth_OU_Brownian_Diffusion_And_Logistic(~,u, dim, W, L, kk, alpha, CC)
% There's a slight change in the diffusion equation here. It is now
% Ut = (k_bar+beta*dW)(Uxx+Uyy), where k_bar is the average diffusion rate.
m = dim(1);
n = dim(2);
deltaX = W/n;
deltaY = L/m;
...
%%
temp(1,1) = (k_bar+beta.*Xem(1,1)).*((u(1,1+1) -2*u(1,1) + u(1,1))/deltaX.^2 + ...
    (u(1+1,1) -2*u(1,1) + u(1,1))/deltaY.^2) + Xem(1,1)*u(1,1)*(CC - u(1,1));
temp(1,n) = (k_bar+beta.*Xem(1,n)).*((u(1,n) -2*u(1,n) + u(1,n-1))/deltaX.^2 + ...
    (u(1+1,n) -2*u(1,n) + u(1,n))/deltaY.^2) + Xem(1,n)*u(1,n)*(CC - u(1,n));
temp(m,1) = (k_bar+beta.*Xem(m,1)).*((u(m,1+1) -2*u(m,1) + u(m,1))/deltaX.^2 + ...
    (u(m,1) -2*u(m,1) + u(m-1,1))/deltaY.^2) + Xem(m,1)*u(m,1)*(CC - u(m,1));
temp(m,n) = (k_bar+beta.*Xem(m,n)).*((u(m,n) -2*u(m,n) + u(m,n-1))/deltaX.^2 + ...
    (u(m,n) -2*u(m,n) + u(m-1,n))/deltaY.^2) + Xem(m,n)*u(m,n)*(CC - u(m,n));

for i = 2:n-1
    temp(1,i) = (k_bar+beta.*Xem(1,i)).*((u(1,i+1) -2*u(1,i) + u(1,i-1))/deltaX.^2 + ...
        (u(1+1,i) -2*u(1,i) + u(1,i))/deltaY.^2) + Xem(1,i)*u(1,i)*(CC - u(1,i));
    temp(m,i) = (k_bar+beta.*Xem(m,i)).*((u(m,i+1) -2*u(m,i) + u(m,i-1))/deltaX.^2 + ...
        (u(m,i) -2*u(m,i) + u(m-1,i))/deltaY.^2) + Xem(m,i)*u(m,i)*(CC - u(m,i));
end

for j = 2:m-1
    temp(j,1) = (k_bar+beta.*Xem(j,1)).*((u(j,1+1) -2*u(j,1) + u(j,1))/deltaX.^2 + ...
        (u(j+1,1) -2*u(j,1) + u(j-1,1))/deltaY.^2) + Xem(j,1)*u(j,1)*(CC - u(j,1));
    temp(j,n) = (k_bar+beta.*Xem(j,n)).*((u(j,n) -2*u(j,n) + u(j,n-1))/deltaX.^2 + ...
        (u(j+1,n) -2*u(j,n) + u(j-1,n))/deltaY.^2) + Xem(j,n)*u(j,n)*(CC - u(j,n));
end

for i = 2:n-1;
    for j = 2:m-1;
        temp(j, i) =  (k_bar+beta.*Xem(j,i)).*((u(j,i+1) -2*u(j,i) + u(j,i-1))/deltaX.^2 + ...
